...
(
	...
	SELECT 
		CASE WHEN 
			st.[text] LIKE '%StudentCoursesStage%' OR 
			st.[text] LIKE '%CourseMasterStage%' OR 
			st.[text] LIKE '%StudentStage%' THEN 'SISStaging3' 
			ELSE ISNULL(DB_NAME(st.dbid), 'cc3') END AS [DatabaseName],
		   ...
		   CASE 
			  WHEN st.[text] LIKE '%CREATE PROCEDURE%' THEN 'Procedure'
		   ...
		   REPLACE(
			REPLACE(
				REPLACE(
					CASE 
						WHEN st.[text] LIKE '%CREATE PROCEDURE%' THEN TRIM(SUBSTRING(st.[text], CHARINDEX('CREATE PROCEDURE', st.[text]) + LEN('CREATE PROCEDURE'), 10000))
				...
				'[', ''),
			']', ''),
			char(10), ''),char(13), ' '), char(9), ' ') AS SQL

	FROM sys.dm_exec_query_stats qs
	CROSS APPLY sys.dm_exec_sql_text(sql_handle) st
	WHERE 
		total_worker_time > 0 AND 
		st.[text] NOT LIKE '%msparam%' AND 
		st.[text] NOT LIKE '%sys.sp_table%' AND 
		st.[text] NOT LIKE '%sp_sqlagent%' AND 
		st.[text] NOT LIKE '%sp_columns%' AND 
		st.[text] NOT LIKE '%tbl_fedauth%' AND
		st.[text] NOT LIKE '%Tbl1%' AND 
		st.[text] NOT LIKE '%Col1%' AND 
		st.[text] NOT LIKE '%SCHEMA_NAME%' AND 
		st.[text] NOT LIKE '%tempdb.dbo%' AND 
		st.[text] NOT LIKE '%COL_VAL(object%' AND
		st.[text] NOT LIKE '%vwQueryStats%' AND 
		st.[text] NOT LIKE '%size_in_mb%' AND 
		st.[text] NOT LIKE '%threshold_time%' AND 
		st.[text] NOT LIKE '%@csn bigint%' AND
		st.[text] NOT LIKE '%msdb.%' AND 
		st.[text] NOT LIKE '%Cloud Lifter%'
...
GROUP BY DatabaseName, [Type] , TRIM(CASE WHEN [Type] != 'Other' THEN SUBSTRING([SQL], 1, CHARINDEX(' ', [SQL])) ELSE [SQL] END)
