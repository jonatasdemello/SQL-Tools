DECLARE @TableName VARCHAR(MAX) = 'Icon' -- Replace 'NewsItem' with your table name
DECLARE @TableSchema VARCHAR(MAX) = 'CMS' -- Replace 'Markets' with your schema name
...
SET @result = @result + CHAR(9) + CHAR(9) + '#region Instance Properties' + CHAR(13)  
SELECT 
	@result = @result + /* CHAR(13) + */
	CHAR(9) + CHAR(9) + ' public ' + ColumnType + ' ' + ColumnName + ' { get; set; } ' + CHAR(13) 
FROM
(
    SELECT  c.COLUMN_NAME AS ColumnName, CASE c.DATA_TYPE   
            WHEN 'bigint' THEN CASE C.IS_NULLABLE WHEN 'YES' THEN 'Int64?' ELSE 'Int64' END
            WHEN 'binary' THEN 'Byte[]'
            WHEN 'bit' THEN CASE C.IS_NULLABLE WHEN 'YES' THEN 'Boolean?' ELSE 'Boolean' END            
            ...
            WHEN 'float' THEN CASE C.IS_NULLABLE WHEN 'YES' THEN 'Single?' ELSE 'Single' END                                    
            WHEN 'image' THEN 'Byte[]'
            WHEN 'int' THEN CASE C.IS_NULLABLE WHEN 'YES' THEN 'Int32?' ELSE 'Int32' END
            ...
            WHEN 'smallint' THEN CASE C.IS_NULLABLE WHEN 'YES' THEN 'Int16?' ELSE 'Int16'END            
            ...
            WHEN 'timestamp' THEN CASE C.IS_NULLABLE WHEN 'YES' THEN 'DateTime?' ELSE 'DateTime' END                                    
            WHEN 'tinyint' THEN CASE C.IS_NULLABLE WHEN 'YES' THEN 'Byte?' ELSE 'Byte' END                                                
            WHEN 'uniqueidentifier' THEN 'Guid'
            WHEN 'varbinary' THEN 'Byte[]'
            WHEN 'varchar' THEN 'String' ELSE 'Object'
        END AS ColumnType
        , c.ORDINAL_POSITION 
...
SET @result = @result + CHAR(13) + CHAR(9) + CHAR(9) + '#endregion Instance Properties' + CHAR(13)  
