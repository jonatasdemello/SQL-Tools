...
Delete Procedure Creation Logic

Generates a drop if exists statement
Generates a parameter list for just the primary key in the table
Generates and Delete Statement
...
-- delete
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[createDeleteSP]') AND type in (N'FN', N'IF', N'TF', N'FS', N'FT'))
DROP FUNCTION [dbo].[createDeleteSP]
GO
