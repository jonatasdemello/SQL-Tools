...
foreach ($filename in Get-ChildItem -Path $FolderPath -Filter "dataset*.sql") { Invoke-Sqlcmd -ServerInstance ".\SQLEXPRESS" -Database "CC3_CMS_local" -InputFile $filename}

foreach ($filename in Get-ChildItem -Path $FolderPath -Filter "dataset*.sql") { Invoke-Sqlcmd -ServerInstance ".\SQLEXPRESS" -Database "CC3_CMS_local" -InputFile $filename ; "$filename" }
