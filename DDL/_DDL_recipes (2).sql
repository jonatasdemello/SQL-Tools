...
-- remove old sproc family.OverallLessonProgressByStudent
IF EXISTS(SELECT * FROM information_schema.routines
	WHERE routine_type = 'PROCEDURE' 
		and routine_schema = 'Family' 
		and routine_name = 'OverallLessonProgressByStudent'	)
BEGIN
	DROP PROCEDURE Family.OverallLessonProgressByStudent
END
GO

-- remove never used ancient tables inside Student schema
IF EXISTS(SELECT * FROM information_schema.tables
	WHERE table_schema = 'Student' AND table_name = 'ParentStudent'	)
BEGIN
	DROP TABLE Student.ParentStudent;
END
GO
...
BEGIN
	...
	-- or
	ALTER TABLE Student.StudentProfile ADD CONSTRAINT [DF__StudentProfile__CreatedDateUTC]  DEFAULT (sysutcdatetime()) FOR [CreatedDateUTC]
	ALTER TABLE Student.StudentProfile ADD CONSTRAINT [DF__StudentProfile__ModifiedDateUTC]  DEFAULT (sysutcdatetime()) FOR [ModifiedDateUTC]
END
...
ALTER TABLE [dbo].[TestTable] 
	ADD CONSTRAINT [PK_TestTable] PRIMARY KEY CLUSTERED ([ID] ASC)
GO
...
BEGIN
    DROP TABLE Student.StudentProfile
END
...
where 
	t.name = 'FamilyProfile'
    and s.name = 'Family'
    and c.name = 'InviteStatusId'
-- type: 'D'
...
SELECT top 10 * FROM information_schema.TABLE_CONSTRAINTS
    WHERE constraint_type not in ('FOREIGN KEY' ,'PRIMARY KEY', 'CHECK', 'UNIQUE')
	or constraint_name like 'DF%'
